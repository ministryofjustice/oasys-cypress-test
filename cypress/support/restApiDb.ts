...
import { DbOffenderWithAssessments, DbAssessment, DbVictim, DbOffence, DbRsr, DbSection, DbAction, DbObjective, DbBspObjective, DbNeed } from 'restApi/dbClasses'
 ...
 */
export async function getOffenderWithAssessments(crnDetails: { crnSource: Provider, crn: string }): Promise<DbOffenderWithAssessments> {
    ...
    // Get offender data
    const offenderData = await db.selectData(DbOffenderWithAssessments.query(crnDetails.crnSource, crnDetails.crn))
    ...
    const assessmentData = await db.selectData(DbAssessment.query(crnDetails.crnSource, crnDetails.crn))
    ...
    // Add standalone RSRs
    const rsrData = await db.selectData(DbRsr.query(crnDetails.crnSource, crnDetails.crn))
