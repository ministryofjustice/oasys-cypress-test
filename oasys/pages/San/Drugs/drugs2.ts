...
export class Drugs2 extends BaseSanEditPage {
    ...
    drugType = new SanElement.CheckboxGroup<'amphetamines' | 'benzodiazepines' | 'cannabis' | 'cocaine' | 'crack' | 'ecstasy' | 'hallucinogenics' | 'heroin' | 'methadone' | 'prescribed' | 'opiates' | 'solvents' | 'spice' | 'steroids' | 'other'>('#select_misused_drugs', ['amphetamines', 'benzodiazepines', 'cannabis', 'cocaine', 'crack', 'ecstasy', 'hallucinogenics', 'heroin', 'methadone', 'prescribed', 'opiates', 'solvents', 'spice', 'steroids', 'other'])
    ...
    methadoneLastSixMonths = new SanElement.Radiogroup<'yes' | 'no'>('#drug_last_used_methadone', ['yes', 'no'])
    prescribedLastSixMonths = new SanElement.Radiogroup<'yes' | 'no'>('#drug_last_used_prescribed', ['yes', 'no'])
