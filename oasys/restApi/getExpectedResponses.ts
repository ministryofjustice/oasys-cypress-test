...
/**
 * Fetch all relevant data needed to build the expected API responses for a single offender.
 * 
 * Returns a DbOffenderWithAssessments object via a result alias.
 */
export function getOffenderWithAssessments(crnSource: Provider, crn: string, resultAlias: string) {

    cy.task('getOffenderWithAssessments', { crnSource: crnSource, crn: crn }).then((result: dbClasses.DbOffenderWithAssessments) => {
 ...
 */
export function getExpectedResponses(offenderData: dbClasses.DbOffenderWithAssessments, parameters: EndpointParams[], resultAlias: string) {

    const results: (EndpointResponse | RestErrorResult)[] = []

    parameters.forEach((params) => {
        getSingleResponse(offenderData, params, 'singleResultAlias')
        cy.get<EndpointResponse | RestErrorResult>('@singleResultAlias').then((result) => {
            results.push(result)
        })
    })
    cy.wrap(results).as(resultAlias)
}


function getSingleResponse(offenderData: dbClasses.DbOffenderWithAssessments, parameters: EndpointParams, resultAlias: string) {
    ...
    f(offenderData, parameters, resultAlias)
