    ...
    it('Part 3', () => {
        ...
        cy.task('retrieveValue', 'offender').then((offenderData) => {
            ...
            cy.get<number[]>('@result').then((pks) => {
                ...
                oasys.Db.getData(`select to_char(lastupd_date, 'YYYY-MM-DD HH24:MI:SS')  from oasys_set where oasys_set_pk = ${pks[0]}`, 'lastUpdDate')
                cy.get<string[][]>('@lastUpdDate').then((initialData) => {
                    ...
                    const sectionQuery = `select count(*) from oasys_section where oasys_set_pk = ${pks[0]} 
